...
bool PluginBase::InputKeyBind() {
	if (keybindVisual) {
		...
		if (frameKeysOneUp) {
			keybindVisual->keys = keybindVisual->newKeys;
			keybindVisual = nullptr;
		}
		return true;
	}
...
void PluginBase::Render()
{
	...
	if (optionWindow->Begin()) {
		...
		if (ImGui::CollapsingHeader(MAIN_INFO)) {
			if (RenderInputText("chainload dll", chainLoad, 64, TEXTSPACE)) {
			...
			if (RenderCheckbox("Require keybind", &requiresInteraction)) {
				Config::SaveBool(MAIN_INFO, REQUIRES_INTERACTION_KEYBIND, requiresInteraction);
				Config::Save();
			}
			RenderKeyBind(interactionKeyBind);
			if (RenderCheckbox("Enable TP api", &tpApiEnabled)) {
				Config::SaveBool(MAIN_INFO, ENABLE_TP_API, tpApiEnabled);
				Config::Save();
			}
			if (tpApiEnabled) {
				if (RenderSliderInt("TP upd. interval", &tpUpdateInterval, MIN_TP_INTERVAL, MAX_TP_INTERVAL)) {
					Config::SaveLong(MAIN_INFO, TP_UPDATE_INTERVAL, tpUpdateInterval);
					Config::Save();
				}
				if (RenderInputText("URL items info", configItemsUrl, 128, TEXTSPACE)) {
					Config::SaveText(MAIN_INFO, URL_ITEMS_INFO, configItemsUrl.c_str());
					Config::Save();
				}
				if (RenderInputText("URL listings info", configPricesUrl, 128, TEXTSPACE)) {
