...
void IFilter::Render()
{
	...
	if (isOpened) {
		...
		ImGui::Text("Inverted : ");
		ImVec4 colorOn = invert ? Addon::Colors[AddonColor_PositiveText] : ImGui::GetStyle().Colors[ImGuiCol_Text];
		ImVec4 colorOff = !invert ? Addon::Colors[AddonColor_PositiveText] : ImGui::GetStyle().Colors[ImGuiCol_Text];
		RenderTooltip(std::vector<TooltipColor>{TooltipColor("ON", colorOn),
			TooltipColor("The output from the filter is inverted, every filtered item will not be filtered and vice versa.", colorOn),
			TooltipColor("\n"),
			TooltipColor("OFF", colorOff),
			TooltipColor("The output from the filter is not changed.", colorOff)});
