...
#define DISABLE_SELL "disableSell"
...
private:
	...
	KeyBindData* openInfoWindow = new KeyBindData();
	bool advancedOptions = false;
	bool ascendedRarity = false;
	bool sellDisabled = false;
	...
	////SKIP IDS
	int curRetry = 0;
	const int defaultRetry = 30;
	int lastSlot = -1;
	std::vector<int> skipUnsellableIds;

	void HookVendorFunc();
	void RenderMenu();
	void RenderItemsFiltered(const char* text);
	void UpdateFilter();
